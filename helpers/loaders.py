...
def _getdatatransformswm(datatype):
    if datatype.lower() == CIFAR10 or datatype.lower() == CIFAR100:
...
def getwmloader(datatype,wm_path, batch_size, labels_path): #重要水印标签语句
    transform_wm = _getdatatransformswm(datatype)
    ...
    wmset = ImageFolderCustomClass(
        ...
        transform_wm)
    img_nlbl = []
    wm_targets = np.loadtxt(os.path.join(wm_path, labels_path))  # 路径拼接
...
def _getdatatransformswmtest(datatype):
    if datatype.lower() == CIFAR10 or datatype.lower() == CIFAR100:
        transform_wm = transforms.Compose([
            transforms.CenterCrop(32),
#             transforms.ToTensor(),
...
def getwmtestloader(datatype,wm_path, batch_size, labels_path): #测试水印标签语句
    transform_wm = _getdatatransformswmtest(datatype)
    # load watermark images
    wmloader = None
